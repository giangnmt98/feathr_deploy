...
class _FeathrLocalSparkJobLauncher(SparkJobLauncher):
    ...
    def submit_feathr_job(
        ...
        maven_dependency_without_feathr = f"{cfg.pop('spark.jars.packages', self.packages)},{plus_packages}"
        cfg = configuration.copy() if configuration else {}
        plus_packages = cfg.pop('spark.jars.plus.packages', ",".join([]))
        ...
        if not main_jar_path:
            ...
            raise Exception("VNPT don't use Maven, contact admin to get jar file!!!")
            if not python_files:
                # This is a JAR job
        ...
        cmds = split(cmd)
        if properties:
            cmds.extend(["--system-properties", json.dumps(properties)])
        if output:
            cmds.extend(["--output", output])

        proc = Popen(cmds, shell=False, stdout=log_append)
    ...
    def _get_default_package(self):
        ...
        packages.append("org.apache.hadoop:hadoop-mapreduce-client-core:3.3.2")
        packages.append("org.apache.hadoop:hadoop-common:3.3.2")
