...
from feathr.definition.source import HdfsSource, InputContext, JdbcSource, MongoDbSource, Source
    ...
    def _extract_features_from_context(self, anchor_list, derived_feature_list, result_path: Path) -> RepoDefinitions:
        ...
        for derived_feature in derived_feature_list:
            if isinstance(derived_feature, DerivedFeature):
                ...
                raise RuntimeError(
                    f"Please make sure you pass a list of `DerivedFeature` objects to the `derived_feature_list` argument. {str(type(derived_feature))} is detected."
                )
        ...
        for anchor in anchor_list:
            ...
            for feature in vars(anchor)["features"]:
                ...
                if isinstance(feature, Feature):
                    ...
                    raise RuntimeError(
                        f"Please make sure you pass a list of `Feature` objects. {str(type(feature))} is detected."
                    )
...
"""
        ...
        anchored_feature_configs = tm.render(
            feature_anchors=repo_definitions.feature_anchors, sources=repo_definitions.sources
        )
        config_file_path = os.path.join(local_workspace_dir, config_file_name)
