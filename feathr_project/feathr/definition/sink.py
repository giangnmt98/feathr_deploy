...
class JdbcSink(Sink):
    ...
    def get_required_properties(self):
        add = ["%s_DRIVER" % self.name.upper(), "%s_VERSION" % self.name.upper()]
        if not hasattr(self, "auth"):
            return [] + add
        if self.auth == "USERPASS":
            return ["%s_USER" % self.name.upper(), "%s_PASSWORD" % self.name.upper()] + add
        elif self.auth == "TOKEN":
            return ["%s_TOKEN" % self.name.upper()] + add
        ...
        tm = Template(
            """  
            {
                ...
                params: {
                    ...
                    driver: "${{ "{" }}{{sink.name}}_DRIVER{{ "}" }}"
                    version: "${{ "{" }}{{sink.name}}_VERSION{{ "}" }}"
    ...
    def to_argument(self):
        ...
        d["driver"] = "${" + self.name.upper() + "_DRIVER}"
        d["version"] = "${" + self.name.upper() + "_VERSION}"
...
class MongoDbSink(Sink):
    def __init__(self, name: str, url: str, collection_name: str) -> None:
        self.name = name
        self.url = url
        ...
        tm = Template(
            """  
            {
                ...
                params: {
                    ...
                    user: "${{ "{" }}{{sink.name}}_USER{{ "}" }}"
                    password: "${{ "{" }}{{sink.name}}_PASSWORD{{ "}" }}"
                    collection: "{{sink.collection_name}}"
                }
            }
        """
        )
...
class ElasticSearchSink(GenericSink):
    ...
    def __init__(self, name: str, host: str, index: str, ssl: bool = True, auth: bool = True, mode="OVERWRITE"):
        """
