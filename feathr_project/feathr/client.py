...
from feathr.definition.sink import HdfsSink, Sink, RedisSink, MongoDbSink
...
from feathr.utils.crud import SinkCRUD, RedisCRUD, MongoDBCRUD
...
class FeathrClient(object):
    ...
    def __init__(
        ...
        if self.env_config.get("online_store__redis__host"):
            ...
            self.redis_cluster_mode = self.env_config.get("online_store__redis__cluster_mode")
            self._construct_redis_client()

        self.mongodb_enabled = bool(self.env_config.get("online_store__mongodb__host"))

        if self.mongodb_enabled:
            spec = importlib.util.find_spec("pymongo")
            if spec is None:
                self.logger.warning(
                    'You have configured Mongodb host, but there is no local Mongodb client package. Install the package using "pip install pymongo". '
                )
            self.mongodb_host = self.env_config.get("online_store__mongodb__host")
            self.mongodb_port = self.env_config.get("online_store__mongodb__port")
            self.mongodb_database = self.env_config.get("online_store__mongodb__database")
            self._construct_mongodb_client()
    ...
    def get_online_features_by_crud(
            self,
            crud: SinkCRUD,
            feature_table: str,
            key: str,
        ...
        """
        Retrieve and decode features for a given key from a feature table.

        Args:
            crud: An instance of SinkCRUD for interacting with the data store.
            feature_table: The name of the feature table.
            key: The unique key associated with the data.
            feature_names: List of feature names to retrieve.

        Returns:
            A list of feature values in the same order as feature_names.
        """
        data_in_feature_table = crud.read(feature_table=feature_table, input_key=key, feature_names=feature_names)
        if not data_in_feature_table:
            logger.warning(f"No data found for key '{key}' in feature table '{feature_table}'.")
            return [None] * len(feature_names)

        return self._decode_proto(data_in_feature_table)

    def get_online_features(self, feature_table: str, key: Any, feature_names: List[str], sink: Sink = None) -> Dict[
        str, Any]:
        feature_data = {}
        if sink is None or isinstance(sink, RedisSink):
            redis_crud = RedisCRUD(
                client=self.redis_client,
                is_cluster=self.redis_cluster_mode
            )
            feature_data = self.get_online_features_by_crud(
                crud=redis_crud,
                feature_table=feature_table,
                key=key,
                feature_names=feature_names
            )
        elif isinstance(sink, MongoDbSink):
            mongodb_crud = MongoDBCRUD(
                collection=self.mongodb_db[feature_table]
            )
            feature_data = self.get_online_features_by_crud(
                crud=mongodb_crud,
                feature_table=feature_table,
            ...
            raise ValueError("Unsupported sink type provided.")

        return feature_data

    def multi_get_online_features(
            self, feature_table: str, keys: List[Any], feature_names: List[str], sink: Sink = None
    ) -> Dict[str, List[Any] ]:
        feature_data = {}
        if sink is None or isinstance(sink, RedisSink):
            redis_crud = RedisCRUD(
            ...
            feature_data = self.multi_get_online_features_by_crud(
                crud=redis_crud,
                feature_table=feature_table,
                keys=keys,
                feature_names=feature_names
    ...
    def multi_get_online_features_by_crud(
            self, crud: SinkCRUD, feature_table: str, keys: List[Any], feature_names: List[str]
    ) -> Dict[str, List[Any]]:
        """
        Retrieve and decode online features for multiple keys using the specified CRUD.

        Args:
            crud: The CRUD instance (e.g., RedisCRUD or MongoDBCRUD).
            feature_table: The name of the feature table.
            keys: A list of keys for which features are to be retrieved.
            feature_names: A list of feature names to retrieve.

        Returns:
            A dictionary mapping keys to lists of feature values in the order of feature_names.
        """
        try:
            raw_data_list = crud.multi_read(feature_table=feature_table, keys=keys, feature_names=feature_names)
            decoded_data_list = [self._decode_proto(raw_data) if raw_data else [None] * len(feature_names)
                                 for raw_data in raw_data_list]
            results = {
                key: decoded_data if decoded_data else [None] * len(feature_names)
                for key, decoded_data in zip(keys, decoded_data_list)
            }

            return results
        except Exception as e:
            logger.error(f"Error retrieving and decoding online features: {e}")
            return {}
    ...
    def _construct_redis_client(self):
        ...
        if cluster_mode:
            ...
            self.logger.info("Redis Cluster connected !!!")
        else:
    ...
    def _construct_mongodb_client(self):
        try:
            mongodb_user = self.env_config.get_from_env_or_akv("MONGODB_USER")
            mongodb_password = self.env_config.get_from_env_or_akv("MONGODB_PASSWORD")

            if not self.mongodb_enabled:
                logger.info("MongoDB is not enabled. Skipping MongoDB initialization.")
                return None

            if not all([self.mongodb_host, self.mongodb_port, self.mongodb_database]):
                raise RuntimeError(
                    "Incomplete MongoDB configuration. Please provide 'mongodb__host', 'mongodb__port', "
                    "'mongodb__database', 'mongodb__user', and 'mongodb__password' in feathr_config.yaml or environment variables."
                )

            if mongodb_user and mongodb_password:
                self.mongodb_client = MongoClient(
                    host=self.mongodb_host,
                    port=int(self.mongodb_port),
                    username=mongodb_user,
                    password=mongodb_password
                )
            else:
                logger.warning("MongoDB connection is being established without authentication.")
                self.mongodb_client = MongoClient(
                    host=self.mongodb_host,
                    port=int(self.mongodb_port)
                )

            if self.mongodb_database not in self.mongodb_client.list_database_names():
                logger.info(f"Database '{self.mongodb_database}' does not exist. Creating the database...")
                self.mongodb_db = self.mongodb_client[self.mongodb_database]
                self.mongodb_db.create_collection("default_collection")
                logger.info(f"Database '{self.mongodb_database}' and a default collection have been created.")
            else:
                self.mongodb_db = self.mongodb_client[self.mongodb_database]
                logger.info(f"Successfully connected to existing MongoDB database: {self.mongodb_database}.")
        except Exception as e:
            logger.error(f"Error during MongoDB client construction: {e}")
    ...
    def materialize_features(
        ...
        for sink in settings.sinks:
            if isinstance(sink, HdfsSink):
                hdfs_sinks.append(sink)
            else:
                non_hdfs_sinks.append(sink)
        count = 0
        for end in sorted(settings.get_backfill_cutoff_time()):
            settings.backfill_time.end = end
            copy_settings = copy.copy(settings)
            if len(hdfs_sinks) > 0:
                rename_store_name_sinks = []
                for sink in hdfs_sinks:
                    copy_s = copy.copy(sink)
                    copy_s.store_name = copy_s.store_name+f"{count}"
                    rename_store_name_sinks.append(copy_s)
                copy_settings.sinks = non_hdfs_sinks + rename_store_name_sinks
            config = _to_materialization_config(copy_settings)
            count+=1
    ...
    def _get_adls_config_str(self):
        ...
        config_str = """
            ...
            ADLS_KEY: "{ADLS_KEY}"
            """.format(
            ADLS_ACCOUNT=account, ADLS_KEY=key
        )
        return self._reshape_config_str(config_str)
    ...
    def _get_blob_config_str(self):
        ...
        config_str = """
            ...
            BLOB_KEY: "{BLOB_KEY}"
            """.format(
            BLOB_ACCOUNT=account, BLOB_KEY=key
        )
        return self._reshape_config_str(config_str)
    ...
    def _get_snowflake_config_str(self):
        ...
        config_str = """
            ...
            """.format(
            JDBC_SF_URL=sf_url,
            JDBC_SF_USER=sf_user,
            JDBC_SF_PASSWORD=sf_password,
            JDBC_SF_ROLE=sf_role,
            JDBC_SF_WAREHOUSE=sf_warehouse,
        )
        return self._reshape_config_str(config_str)
    ...
    def _transfer_data_between_sinks(
            self,
            source_crud: SinkCRUD,
            target_crud: SinkCRUD,
            feature_table: str,
            key_pattern: Optional[str] = "*",
            feature_names: Optional[List[str]] = None
    ):
        """
        Generalized function to transfer data from one sink to another.

        Args:
            source_crud: The source CRUD instance (e.g., RedisCRUD, MongoDBCRUD).
            target_crud: The target CRUD instance (e.g., MongoDBCRUD, RedisCRUD).
            feature_table: The name of the feature table to transfer data for.
            key_pattern: Pattern to filter keys in the source sink (default is '*').
            feature_names: List of feature names to transfer. If None, all features will be transferred.

        Returns:
            None
        """
        try:
            logger.info(f"Starting data transfer for feature table '{feature_table}'.")

            # Get all keys matching the pattern from the source sink
            all_keys = source_crud.get_all_keys(feature_table, key_pattern)
            if not all_keys:
                logger.warning(f"No keys found in the source for feature table '{feature_table}'.")
                return

            logger.info(f"Truncating target sink for feature table '{feature_table}'.")
            target_crud.truncate(feature_table)

            if not feature_names:
                logger.info(f"Fetching all features for keys in feature table '{feature_table}'.")
                first_key = all_keys[0]
                redis_key = source_crud._construct_key(feature_table, first_key)
                first_key_data = source_crud.client.hgetall(redis_key)
                if not first_key_data:
                    logger.warning(f"No features found for key '{first_key}'. Cannot determine feature names.")
                    return
                feature_names = list(first_key_data.keys())

            logger.info(f"Transferring features: {feature_names}")
            raw_data = source_crud.multi_read(feature_table, all_keys, feature_names)
            for key, data in zip(all_keys, raw_data):
                if not data:
                    logger.warning(f"No data found for key '{key}'. Skipping.")
                    continue
                target_data = {feature: value for feature, value in zip(feature_names, data)}
                target_crud.create(feature_table, key, target_data)

            logger.info(f"Data transfer for feature table '{feature_table}' completed successfully.")
        except Exception as e:
            logger.error(f"Error transferring data for feature table '{feature_table}': {e}")
